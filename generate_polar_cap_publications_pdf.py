...
print("Fetching publications for the polar-cap PDF bibliography...")
...
print(f"Generating PDF for {len(matched_papers)} polar-cap SuperDARN papers...")
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        if self._pageNumber > 1:
            ...
            self.drawString(54, letter[1] - 40, "BIBLIOGRAPHY: POLAR-CAP SUPERDARN RADAR RESEARCH")
...
output_path_artifact = os.path.join(output_dir, 'superdarn_polar_cap_publications.pdf')
output_path_local = 'superdarn_polar_cap_publications.pdf'
...
story.append(Paragraph("Publications Using Polar-Cap SuperDARN Radars (&ge; 70&deg; MLAT)", style_title))
metadata_desc = (
    "This document lists all peer-reviewed publications from the official SuperDARN Canada registry "
    "that utilize data from the 9 polar-cap SuperDARN radars (geomagnetic latitude &ge; 70&deg; absolute). "
    "To ensure scientific accuracy, an operational timeline constraint is applied: papers are only classified "
    "as polar-cap publications if they were published in or after the operational startup year of the respective radar "
    "whose data was used. Citation metadata is retrieved from Crossref."
)
...
print("Polar-cap bibliography PDF generated successfully.")
